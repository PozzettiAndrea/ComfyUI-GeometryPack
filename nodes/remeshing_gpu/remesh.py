    ...
    def define_schema(cls):
        return io.Schema(
            ...
            inputs=[
                ...
                io.Float.Input("remesh_band", default=1.0, min=0.1, max=5.0, step=0.1, tooltip="Thickness of the voxel shell dual-contouring evaluates around the surface, in VOXELS (real width = band x scale/grid_resolution). This is a robustness-vs-detail knob, NOT the main detail knob (that's grid_resolution). 1.0 (default) = a one-voxel shell, correct almost always. 0.5 = hugs the surface: keeps the thinnest walls and sharpest detail, but can leave holes/gaps if the input is noisy or grid_resolution is too low. 2-3 = thicker shell: fills small gaps, more robust on messy / non-watertight input, smoother -- but fuses nearby thin walls and rounds off detail; slower + more VRAM. Above ~3 rarely helps. Rule of thumb: holes in output -> raise band; thin features vanishing -> lower band; want crisper detail -> raise grid_resolution, not band.", optional=True),
